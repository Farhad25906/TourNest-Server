...
router.get("/host/my-tours", auth(UserRole.HOST), TourController.getHostTours);

// Get host tour statistics
router.get("/host/stats", auth(UserRole.HOST), TourController.getHostTourStats);
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      return TourController.completeTour(req, res, next);
    } catch (error) {
      return next(error);
    }
  }
